...
<div class="container px-6 mx-auto grid">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 my-8 animate-fade-in">
        <div>
            <h2 class="text-3xl font-bold text-gray-800 dark:text-gray-100">Kata Sambutan</h2>
            <p class="mt-1 text-gray-600 dark:text-gray-400">Kelola informasi kata sambutan Anda</p>
        </div>
        <a href="{{ route('admin.kata-sambutan.create') }}"
    ...
    <div class="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        @forelse ($kataSambutans as $kataSambutan)
        <div class="bg-white rounded-lg shadow-md dark:bg-gray-800 hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden">
            ...
            <div class="relative overflow-hidden aspect-video">
                ...
                <img src="{{ asset('storage/' . $kataSambutan->image) }}" alt="Gambar {{ $kataSambutan->title }}"
            ...
            <div class="p-6 flex-grow">
                <div class="description-container">
                    <div class="text-gray-700 dark:text-gray-300 text-sm leading-relaxed">
                        {!! Str::limit($kataSambutan->description, 120) !!}
                    </div>
                </div>
            </div>
            ...
            <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700 border-t border-gray-100 dark:border-gray-600 flex justify-end gap-4">
                <!-- Edit Button -->
                <a href="{{ route('admin.kata-sambutan.edit', $kataSambutan->id) }}"
                ...
                <!-- Delete Button -->
                <form id="delete-form-{{ $kataSambutan->id }}" action="{{ route('admin.kata-sambutan.destroy', $kataSambutan->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="button" onclick="confirmDelete({{ $kataSambutan->id }})"
        ...
        <div class="col-span-full p-12 bg-white dark:bg-gray-800 rounded-xl shadow text-center">
            <div class="flex flex-col items-center">
                ...
                <p class="text-gray-600 dark:text-gray-400 text-lg">Belum ada data kata sambutan yang tersedia.</p>
                <a href="{{ route('admin.kata-sambutan.create') }}" 
...
function confirmDelete(id) {
    Swal.fire({
        title: 'Yakin ingin menghapus kata sambutan ini?',
