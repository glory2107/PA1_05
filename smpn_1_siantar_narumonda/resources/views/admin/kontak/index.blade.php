...
@section('content')
@php
    // Mapping icon class ke nama yang user-friendly
    $iconNames = [
        'fab fa-whatsapp' => 'Whatsapp',
        'fab fa-instagram' => 'Instagram',
        'fab fa-facebook-f' => 'Facebook',
        'fab fa-youtube' => 'Youtube',
        'fas fa-envelope' => 'Gmail',
        'fas fa-phone-alt' => 'Telepon',
        'fas fa-road' => 'Alamat',
    ];
@endphp
...
<div class="container px-6 mx-auto grid">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 my-8 animate-fade-in">
        <div>
            ...
            <p class="mt-1 text-gray-600 dark:text-gray-400">Kelola informasi kontak sekolah</p>
        </div>
    ...
    <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        <div class="bg-white rounded-lg shadow-md dark:bg-gray-800 hover:shadow-xl transition-all duration-300 flex flex-col">
            <!-- Card Header -->
            <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700 border-b border-gray-100 dark:border-gray-600">
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white truncate flex items-center gap-2">
                    <i class="{{ $kontak->icon }} text-purple-600"></i>
                    <span class="ml-2">{{ $iconNames[$kontak->icon] ?? $kontak->icon }}</span> <!-- Menambahkan margin kiri (ml-2) pada span -->
                </h3>
                <p class="text-sm text-gray-500 dark:text-gray-300">Status: {{ ucfirst($kontak->status) }}</p>
            </div>
            ...
            <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700 border-t border-gray-100 dark:border-gray-600 flex justify-end gap-4">
                ...
                <form id="delete-form-{{ $kontak->id }}" action="{{ route('admin.kontak.destroy', $kontak->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="button"
        ...
        <div class="col-span-full p-12 bg-white dark:bg-gray-800 rounded-xl shadow text-center">
            <div class="flex flex-col items-center">
                ...
                <p class="text-gray-600 dark:text-gray-400 text-lg">Belum ada data kontak yang tersedia.</p>
                <a href="{{ route('admin.kontak.create') }}" 
