...
<div class="container px-6 mx-auto grid">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 my-8 animate-fade-in">
        <div>
            <h2 class="text-3xl font-bold text-gray-800 dark:text-gray-100">Edit Sejarah Sekolah</h2>
            <p class="mt-1 text-gray-600 dark:text-gray-400">Perbarui informasi sejarah sekolah Anda</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-md dark:bg-gray-800 overflow-hidden">
        ...
        <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700 border-b border-gray-100 dark:border-gray-600">
            <h3 class="text-lg font-medium text-gray-800 dark:text-white">Formulir Edit Sejarah Sekolah</h3>
        </div>

        <!-- Form -->
        <form action="{{ route('admin.sejarah-sekolah.update', $sejarahSekolah->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                <label for="title" class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                    Judul Sejarah <span class="text-red-500">*</span>
                </label>
                <input type="text" id="title" name="title" value="{{ old('title', $sejarahSekolah->title) }}"
            ...
            <div class="mb-6">
                <label for="image" class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                    Gambar Sejarah (Opsional)
                </label>
                @if ($sejarahSekolah->image)
                    <div class="mb-4">
                        <img src="{{ asset('storage/' . $sejarahSekolah->image) }}" alt="Gambar Saat Ini"
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" rows="8"
                          class="bg-gray-50 border border-gray-300 text-gray-900 text-base rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-3 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500"
                          required>{{ old('description', $sejarahSekolah->description) }}</textarea>
            </div>
