...
<div class="container px-6 mx-auto grid">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 my-8 animate-fade-in">
        <div>
            <h2 class="text-3xl font-bold text-gray-800 dark:text-gray-100">Edit Struktur Organisasi</h2>
            <p class="mt-1 text-gray-600 dark:text-gray-400">Perbarui informasi struktur organisasi Anda</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-md dark:bg-gray-800 overflow-hidden">
        ...
        <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700 border-b border-gray-100 dark:border-gray-600">
            <h3 class="text-lg font-medium text-gray-800 dark:text-white">Formulir Edit Struktur Organisasi</h3>
        </div>

        <!-- Form -->
        <form action="{{ route('admin.struktur-organisasi.update', $struktur->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                <label for="image" class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                    Gambar Struktur Organisasi (Opsional)
                </label>
                @if ($struktur->image)
                    <div class="mb-4">
                        <img src="{{ asset('storage/' . $struktur->image) }}" alt="Gambar Saat Ini"
